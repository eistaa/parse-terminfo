...
function parseTerminfo(buffer, term) {
    ...
    for (
        ...
        if ( boolean )
            result.capabilities[ALL_VARS[VARORDER.booleans[i]]] = true;
    }
    ...
    for (
        ...
        if ( number !== -1 )
            result.capabilities[ALL_VARS[VARORDER.numbers[i]]] = number;
    }
    ...
    for (
        ...
        if ( tableOffset !== -1 ) {
            ...
            result.capabilities[ALL_VARS[VARORDER.strings[i]]] = buffer.toString(
